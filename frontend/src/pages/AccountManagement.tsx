...
const AccountManagement: React.FC = () => {
  ...
  const toggleAvailabilityMutation = useMutation({
    mutationFn: async (isAvailable: boolean) => {
      const response = await axios.put("/api/auth/availability", {
        isAvailable,
      });
      return response.data;
    },
      ...
      {userData && userData.role === "interpreter" && (
        <div className="mt-8 max-w-md mx-auto bg-white p-6 rounded-lg shadow-md">
          <h2 className="text-2xl font-semibold mb-4">
            Availability / Holiday Mode
          </h2>
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-gray-700">
              {userData.isAvailable
                ? "Available for new bookings"
                : "On Holiday (not accepting new bookings)"}
            </span>
            <button
              onClick={() =>
                toggleAvailabilityMutation.mutate(!userData.isAvailable)
              }
              disabled={
                toggleAvailabilityMutation.isPending || isLoadingUserData
              }
              className={`relative inline-flex items-center h-6 rounded-full w-11 transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 ${
                userData.isAvailable ? "bg-green-500" : "bg-gray-400"
              }`}
            >
              <span className="sr-only">Toggle Availability</span>
              <span
                className={`inline-block w-4 h-4 transform bg-white rounded-full transition-transform duration-200 ease-in-out ${
                  userData.isAvailable ? "translate-x-6" : "translate-x-1"
                }`}
              />
            </button>
          </div>
