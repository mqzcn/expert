...
export const sendContactFormEmailToAdmins = async ({
  senderName,
  senderEmail,
  subject,
  message,
  ...
  try {
    ...
    if (!adminEmails || adminEmails.length === 0) {
      console.warn(
        `No admin emails found to send contact message. Contact form submission from ${senderEmail} will not be delivered to admins.`
      );
    ...
    if (result.error) {
      console.error(
        "Resend error when sending contact form to admins:",
        result.error
      );
    }
  } catch (error) {
